/****************************************************
 * File: cpp_driver_b6.cpp
 * Desc: B6 IMU serial reader & parser (C++17, cross-platform)
 *
 * Build (Linux):
 *   g++ -std=c++17 cpp_driver_b6.cpp -o cpp_driver_b6 -lboost_system -lpthread
 *
 * Build (Windows / MSVC):
 *   cl /EHsc /std:c++17 cpp_driver_b6.cpp /I"path\\to\\boost" /link /LIBPATH:"path\\to\\boost\\lib" ws2_32.lib
 *
 * Run:
 *   ./cpp_driver_b6                # Linux defaults: /dev/ttyUSB0 @ 921600
 *   ./cpp_driver_b6 --port /dev/ttyUSB0 --baud 921600
 *   cpp_driver_b6.exe              # Windows defaults: COM3 @ 921600
 *   cpp_driver_b6.exe --port COM5 --baud 921600
 *
 * Args:
...
static int  g_idx = 0;                 // index inside frame
...
// Print when 0x51/0x52/0x53 all received
...
static void print_if_ready() {
    if (!(g_have_51 && g_have_52 && g_have_53)) return;
    ...
    g_have_51 = g_have_52 = g_have_53 = false;
}
...
static void feed_byte(uint8_t b) {
    ...
    uint8_t* f = g_buf.data();          // 11 bytes
    if (!checksum_ok(f)) {
        // silent on checksum fail to avoid spam
        reset_frame();
        return;
    }
    ...
    switch (type) {
        ...
        default:
            // ignore other types (e.g., 0x54) silently
            break;
    }
...
#else
    return "/dev/ttyUSB0";    // common default on Linux
#endif
}
